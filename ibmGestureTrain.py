...
def genLoihiParams(net):
	...
	np.save('Trained_ibm_test/fc1.npy'  , fc1Weights)
	np.save('Trained_ibm_test/fc2.npy'  , fc2Weights)
	np.save('Trained_ibm_test/conv1.npy', conv1Weights)
	np.save('Trained_ibm_test/conv2.npy', conv2Weights)
	np.save('Trained_ibm_test/pool1.npy', pool1Weights)
	np.save('Trained_ibm_test/pool2.npy', pool2Weights)
	np.save('Trained_ibm_test/pool3.npy', pool3Weights)
...
if __name__ == '__main__':
	...
	# # create dataloaders
	# trainLoader = DataLoader(train_ds, shuffle=True, batch_size=4, num_workers=1)
	# testLoader = DataLoader(test_ds, shuffle=False, batch_size=4, num_workers=1)
	...
	for epoch in range(500):
		...
		# Update stats.
		stats.update()
		stats.plot(saveFig=True, path='Trained_ibm_test/')
		if stats.training.bestLoss is True: torch.save(net.state_dict(), 'Trained_ibm_test/ibmGestureNet.pt')

	# Save training data
	stats.save('Trained_ibm_test/')
	net.load_state_dict(torch.load('Trained_ibm_test/ibmGestureNet.pt'))
	genLoihiParams(net)
